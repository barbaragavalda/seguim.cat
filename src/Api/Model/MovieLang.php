<?php

namespace Api\Model;

use Api\Model\TheTvdb\Client;
use Api\Model\TheTvdb\Languages;
use Core\Model\Model;
use PDO;

class MovieLang extends Model
{

    private const int TTL_SECONDS = 86400; // 24h

    /**
     * counterpart of SerieLang::syncForLanguage() - same shape and same
     * "NULL row = confirmed no translation" caching
     */
    public function syncForLanguage(int $idMovie, int $tvdbMovieId, string $culture, Client $client): array
    {
        $idAppacmanLang = Languages::idForCulture($culture);
        if ($idAppacmanLang === null) {
            return array('name' => null, 'overview' => null);
        }

        $row = $this->find($idMovie, $idAppacmanLang);
        if ($row === null || $this->isStale($row['synced_at'])) {
            $translation = $client->getMovieTranslation($tvdbMovieId, Languages::tvdbCode($idAppacmanLang));
            $this->upsert($idMovie, $idAppacmanLang, $translation);
            $row = $this->find($idMovie, $idAppacmanLang);
        }

        return array('name' => $row['name'] ?? null, 'overview' => $row['overview'] ?? null);
    }

    private function find(int $idMovie, int $idAppacmanLang): ?array
    {
        $sql    = '
            SELECT *
            FROM movie_lang
            WHERE id_movie = :id_movie AND id_appacman_lang = :id_appacman_lang
        ';
        $params = array(
            'id_movie'         => array('value' => $idMovie, 'type' => PDO::PARAM_INT),
            'id_appacman_lang' => array('value' => $idAppacmanLang, 'type' => PDO::PARAM_INT),
        );
        $rows   = $this->mysql->query($sql, $params);
        return $rows[0] ?? null;
    }

    private function isStale(string $syncedAt): bool
    {
        return strtotime($syncedAt) <= (time() - self::TTL_SECONDS);
    }

    private function upsert(int $idMovie, int $idAppacmanLang, ?array $translation): void
    {
        $sql    = '
            INSERT INTO movie_lang (id_movie, id_appacman_lang, name, overview, synced_at)
            VALUES (:id_movie, :id_appacman_lang, :name, :overview, NOW())
            ON DUPLICATE KEY UPDATE
                name = :name_upd, overview = :overview_upd, synced_at = NOW()
        ';
        $name     = $translation['name'] ?? null;
        $overview = $translation['overview'] ?? null;

        $params = array(
            'id_movie'         => array('value' => $idMovie, 'type' => PDO::PARAM_INT),
            'id_appacman_lang' => array('value' => $idAppacmanLang, 'type' => PDO::PARAM_INT),
            'name'             => array('value' => $name, 'type' => PDO::PARAM_STR),
            'name_upd'         => array('value' => $name, 'type' => PDO::PARAM_STR),
            'overview'         => array('value' => $overview, 'type' => PDO::PARAM_STR),
            'overview_upd'     => array('value' => $overview, 'type' => PDO::PARAM_STR),
        );
        $this->mysql->query($sql, $params);
    }

}
